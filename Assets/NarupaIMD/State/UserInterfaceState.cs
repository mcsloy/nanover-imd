...
{
    public class UserInterfaceState : UserState
    {
        [SerializeField]
        ...
        [SerializeField]
        private NarupaCanvas mainMenuScene;
        ...
        private void SwitchCursor(SteamVR_Input_Sources src, bool force = false)
        {
            if ((force || cursorSwitchingEnabled) && src != inputSource)
                SetInputSource(src);
        }
        ...
        {
            applicationStateManager.GotoUserInterfaceState(SteamVR_Input_Sources.RightHand);
            uiManager.GotoScene(mainMenuScene);
        }
    }
}