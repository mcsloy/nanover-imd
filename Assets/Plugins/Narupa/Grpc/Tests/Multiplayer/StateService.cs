...
{
    public class StateService : State.StateBase, IBindableService
    {
        ...
        public override async Task<UpdateLocksResponse> UpdateLocks(UpdateLocksRequest request, ServerCallContext context)
        {
            ...
            foreach (var requestKey in request.LockKeys.Fields.Keys)
                if(locks.ContainsKey(requestKey) && locks[requestKey] != token)
                    return new UpdateLocksResponse { Success = false};
            ...
            return new UpdateLocksResponse { Success = true };
        }

        public override async Task<UpdateStateResponse> UpdateState(UpdateStateRequest request, ServerCallContext context)
        {
            var token = request.AccessToken;
            foreach (var requestKey in request.Update.ChangedKeys.Fields.Keys)
                if(locks.ContainsKey(requestKey) && locks[requestKey] != token)
                    return new UpdateStateResponse { Success = false};
            ...
            return new UpdateStateResponse { Success = true };
        }

        public override async Task SubscribeStateUpdates(SubscribeStateUpdatesRequest request,
                                                   IServerStreamWriter<StateUpdate> responseStream,
        ...
        {
            return Narupa.Protocol.State.State.BindService(this);
        }
