...
namespace NarupaIMD.Selection
{
    public static class VisualisationParser
    {
                                                      ...
                                                      out IMapping<string, Color> mapping)
        {
            ...
            {
                // Object is already a mapping of elements to colors
                case IMapping<string, Color> actualValue:
                    mapping = actualValue;
                    return true;

                // Object is a string name of a predefined mapping
                case string str:
                    mapping = Resources.Load<StringColorMapping>($"Mapping/{str}");
                    return true;

                // Object is a dictionary, to be interpreted as element symbols to colors
                case Dictionary<string, object> dict:
                {
                    var mappingDictionary = new Dictionary<string, Color>();
                    foreach (var (key, val) in dict)
                        if (TryParseString(key, out var str)
                         && TryParseColor(val, out var color))
