    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                // Factor which prevents bowing when normals are perpendicular.
                float factor = 1/sqrt(2 + 4 * (bias - 1) * bias);
               
                v.vertex.xz += factor * (sign(dot1) * a1 * d1 + sign(dot2) * a2 * d2);
                ...
                o.color = ColorSample(pow(curve.startColor, 2.2), pow(curve.endColor, 2.2), bias, _ColorBlend);
                o.bias = bias;
